-- BALCÃO / Malvo production integration.
-- Stores provider-level consent state separately from normalized accounts/transactions.

create table if not exists public.balcao_finance_connections (
  ...
  provider text not null default 'malvo',
  provider_item_id text not null,
  client_user_id text not null,
  institution_name text,
  institution_logo_url text,
  status text not null default 'pending' check (status in ('pending', 'active', 'updating', 'attention', 'error', 'disconnected')),
...
create index if not exists balcao_finance_connections_store_idx
  on public.balcao_finance_connections(store_id, status, updated_at desc);
create index if not exists balcao_finance_connections_business_idx
  on public.balcao_finance_connections(business_id, provider, updated_at desc);

create table if not exists public.balcao_finance_webhook_events (
  id uuid primary key default gen_random_uuid(),
  provider text not null default 'malvo',
...
create index if not exists balcao_finance_webhook_events_item_idx
  on public.balcao_finance_webhook_events(provider_item_id, created_at desc);

alter table public.balcao_finance_connections enable row level security;
alter table public.balcao_finance_webhook_events enable row level security;

revoke all on table public.balcao_finance_connections from public, anon, authenticated;
revoke all on table public.balcao_finance_webhook_events from public, anon, authenticated;
grant all on table public.balcao_finance_connections to service_role;
grant all on table public.balcao_finance_webhook_events to service_role;

-- Expose identifiers only to the authenticated Next.js finance endpoint. They are
-- needed server-side to correlate a consent to the provider-neutral finance rows.
...
begin
  if v_user_id is not null and p_installation_id is not null then
    ...
    if v_store_id is not null then v_authorized := true; end if;
  end if;
  ...
  if not v_authorized
    ...
    from public.balcao_operational_context(p_terminal_id, p_terminal_hash, p_session_id, p_session_hash)
    limit 1;

    if v_operational.current_staff_id is not null and v_operational.current_session_id is not null then
  ...
  select coalesce(jsonb_agg(to_jsonb(a) order by a.created_at asc), '[]'::jsonb)
    into v_accounts
  from (
    select fa.id, fa.institution_name, fa.account_name, fa.account_type, fa.masked_number,
           fa.balance_cents, fa.currency, fa.status, fa.source, fa.last_synced_at, fa.created_at
    from public.balcao_finance_accounts as fa
    where fa.business_id = v_business_id and fa.store_id = v_store_id
  ) as a;

  select coalesce(jsonb_agg(to_jsonb(t) order by t.posted_at desc), '[]'::jsonb)
    into v_transactions
  from (
    select ft.id, ft.account_id, ft.posted_at, ft.amount_cents, ft.description,
           ft.counterparty_name, ft.counterparty_tax_id, ft.category,
           ft.category_confidence, ft.transaction_type, ft.is_internal_transfer, ft.source
    from public.balcao_finance_transactions as ft
    where ft.business_id = v_business_id
      and ft.store_id = v_store_id
      and ft.posted_at >= v_start and ft.posted_at <= now()
  ...
  select coalesce(jsonb_agg(to_jsonb(m) order by m.metric_date asc), '[]'::jsonb)
    into v_metrics
  from (
    select fm.metric_date, fm.sales_cents, fm.cogs_cents, fm.units_sold_milli, fm.source
    from public.balcao_finance_daily_metrics as fm
    where fm.business_id = v_business_id
      and fm.store_id = v_store_id
      and fm.metric_date >= v_start_date and fm.metric_date <= current_date
