-- BALCÃO v0.3: a Google identity owns its own independent BALCÃO account.
-- Business registration data (including CNPJ) is not a global account identity key.
-- Different Google users may therefore create independent businesses with the same tax_id.
