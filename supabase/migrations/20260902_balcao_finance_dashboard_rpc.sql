-- BALCÃO Financeiro: server-facing read model without Vercel service-role dependency.
-- The function validates either the signed-in Google manager or the Balcão
-- terminal + staff session before returning finance data.
...
declare
  ...
  v_start timestamptz := date_trunc('day', now()) - make_interval(days => (case when p_days in (7, 30, 90) then p_days else 30 end) - 1);
  v_start_date date := (date_trunc('day', now()) - make_interval(days => (case when p_days in (7, 30, 90) then p_days else 30 end) - 1))::date;
...
begin
  -- Google management path: the installation identifies the store and auth.uid()
  -- must be an active business member with a management role.
  ...
  -- PIN/staff path: use the same hardened operational credential validation
  -- already used by the rest of Balcão. A terminal alone is insufficient;
  -- an active staff session with Financeiro permission is required.
  if not v_authorized
