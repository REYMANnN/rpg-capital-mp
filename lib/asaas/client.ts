...
export function getAsaasWebhookToken() {
  const explicit = process.env.ASAAS_WEBHOOK_TOKEN_balcao?.trim()
  if (explicit) return explicit
  return createHmac('sha256', requiredApiKey()).update('balcao:asaas:webhook:v1').digest('hex')
}
...
export async function ensureAsaasWebhook(url: string) {
  const listed = await asaasRequest('/webhooks?limit=100')
  const data = Array.isArray(listed.data) ? listed.data as Array<Record<string, any>> : []
  const existing = data.find((row) => row.url === url && row.enabled !== false)
