...
export function requestId(){ return `req_${crypto.randomUUID().replace(/-/g,'')}` }
...
export function paginate<T>(items:T[],url:URL){ const start=cursorIndex(url); const limit=pageSize(url); const data=items.slice(start,start+limit); const next=start+data.length<items.length?Buffer.from(String(start+data.length)).toString('base64url'):null; return {data,meta:{nextCursor:next,limit,total:items.length}} }
