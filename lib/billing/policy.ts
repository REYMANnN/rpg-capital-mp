...
function parseCalendarDate(value: string) {
  ...
  if (date.getUTCFullYear() !== year || date.getUTCMonth() !== month - 1 || date.getUTCDate() !== day) throw new Error('Billing date is invalid')
...
export function buildBillingPlan(createdDate: string): BillingPlan {
  ...
  if (day === 1) {
    return { initialCharge: null, recurring: { amountCents: MONTHLY_AMOUNT_CENTS, firstDueDate: createdDate } }
  }
  return {
    initialCharge: { amountCents: CATCH_UP_AMOUNT_CENTS, dueDate: firstOfShiftedMonth(year, month, 1), maxPayments: 1 },
    recurring: { amountCents: MONTHLY_AMOUNT_CENTS, firstDueDate: firstOfShiftedMonth(year, month, 2) },
  }
}
