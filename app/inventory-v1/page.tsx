...
export const metadata = { title: 'BALCÃO', description: 'Inventário, caixa, financeiro e automações para pequenos negócios' }

export default async function Page() {
  if (process.env.BALCAO_ACCOUNTS_ENFORCED !== 'true') return <InventoryRoleGate role="manager" managementAccess><InventoryV1 /></InventoryRoleGate>
  ...
  if (!context.authorized) { if (context.terminal) redirect('/work'); redirect('/login') }
  ...
  const canAutomations = permissions.has('automations.view')
  ...
  if (canAutomations && !canLoadOperationalState && !canFinance) redirect('/automations')
  if (canFinance && !canLoadOperationalState) return <InventoryRoleGate role={role} managementAccess={false}><FinanceOnlyShell /></InventoryRoleGate>
  if (!canLoadOperationalState && !canFinance) redirect('/work')
