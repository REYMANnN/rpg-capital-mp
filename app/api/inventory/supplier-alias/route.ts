...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await supabase
    ...
    .select('supplier_document,supplier_code,barcode,canonical_name,observed_description,confirmations,revisions,last_seen_at')
...
export async function POST(request: NextRequest) {
  ...
  if (!/^[0-9a-f-]{36}$/i.test(installationId)) {
    return NextResponse.json({ ok: false, error: 'inventory_installation_required' }, { status: 401 })
  }
  ...
  if (![11, 14].includes(supplierDocument.length) || !supplierCode || !/^\d{8,14}$/.test(barcode)) {
    return NextResponse.json({ ok: false, error: 'invalid_alias' }, { status: 400 })
  }

  const supabase = createInventoryCloudClient()
  const { data, error } = await supabase.rpc('inventory_v1_confirm_supplier_alias', {
  ...
  if (error) {
    console.error('inventory_v1_confirm_supplier_alias failed', error)
    return NextResponse.json({ ok: false, error: 'alias_write_failed' }, { status: 500 })
  }
