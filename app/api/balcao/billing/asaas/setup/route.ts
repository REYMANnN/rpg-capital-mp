...
function cleanHolder(value: Partial<AsaasCreditCardHolderInfo> | null | undefined): AsaasCreditCardHolderInfo | null {
  ...
  return {
    name, email, cpfCnpj, postalCode, addressNumber,
...
export async function POST(request: Request) {
  ...
  const { data: business, error: businessError } = await supabase.from('balcao_businesses')
    .select('display_name,tax_id,phone')
  ...
  const { data: current, error: currentError } = await admin.from('balcao_billing_accounts')
    .select('status,asaas_customer_id,asaas_initial_subscription_id,asaas_recurring_subscription_id,first_due_date,next_due_date')
  ...
  if (current && ['configured', 'active'].includes(current.status) && current.asaas_recurring_subscription_id) {
    return NextResponse.json({
      ...
      firstDueDate: current.first_due_date,
  ...
  try {
    ...
    await ensureAsaasWebhook(`${origin}/api/balcao/billing/asaas/webhook`)

    const businessReference = `balcao:business:${store.businessId}`
    const customer = await ensureAsaasCustomer({
      ...
      externalReference: businessReference,
    ...
    const { error: pendingError } = await admin.from('balcao_billing_accounts').upsert({
    ...
    let initialSubscriptionId: string | null = current?.asaas_initial_subscription_id ?? null
    if (plan.initialCharge) {
      ...
      initialSubscriptionId = typeof initial.id === 'string' ? initial.id : null
    ...
    const { error: saveError } = await admin.from('balcao_billing_accounts').update({
      status: 'configured',
      asaas_customer_id: customerId,
      asaas_initial_subscription_id: initialSubscriptionId,
      asaas_recurring_subscription_id: recurringSubscriptionId,
      first_due_date: firstDueDate,
      next_due_date: firstDueDate,
      reconnect_required: false,
    ...
    console.error('BALCAO Asaas billing setup failed', { message: caught instanceof Error ? caught.message : 'unknown error' })
