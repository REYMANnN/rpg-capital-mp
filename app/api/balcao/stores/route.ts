...
export async function POST(request: Request) {
  ...
  const { data: store, error } = await admin.from('inventory_v1_stores').insert({
  ...
  try { await writeAuditEvent({ businessId: input.businessId, storeId: store.id, actorUserId: user.id, action: 'store.created', entityType: 'store', entityId: store.id }) } catch {}
