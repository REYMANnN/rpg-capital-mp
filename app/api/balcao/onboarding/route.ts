...
const WHATSAPP_CONSENT_VERSION = '2026-09-16-v1'
const WHATSAPP_CONSENT_TEXT = 'Aceito receber mensagens da RPG Capital pelo WhatsApp no número informado, incluindo mensagens relacionadas à minha conta, operação da loja, alertas e atendimento. Posso cancelar esse consentimento a qualquer momento respondendo PARAR.'
...
export async function POST(request: Request) {
  ...
  if (data.whatsappConsent) {
    ...
    if (consentError) {
      ...
      return NextResponse.json({ error: 'A loja foi criada, mas não conseguimos registrar sua preferência de WhatsApp. Tente novamente.' }, { status: 500 })
    }
  }
